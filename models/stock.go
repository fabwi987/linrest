...
func (db *DB) GetStocks() ([]*Stock, error) {
	...
	rows, err := db.Query("SELECT id, symbol, created, buyprice, numberofshare, salesprice, name, lasttradeprice, lastupdated FROM stocks")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		tempStock := new(Stock)
		err = rows.Scan(&tempStock.ID, &tempStock.Symbol, &tempStock.Created, &tempStock.BuyPrice, &tempStock.NumberOfShares, &tempStock.SalesPrice, &tempStock.Name, &tempStock.LastTradePriceOnly, &tempStock.LastUpdated)
		if err != nil {
			return nil, err
		}
...
func (db *DB) GetSingleStock(symbol string) (*Stock, error) {
	...
	stmt, err := db.Prepare("SELECT id, symbol, created, buyprice, numberofshare, salesprice, name, lasttradeprice, lastupdated FROM stocks WHERE symbol = ?")
	defer stmt.Close()
